...
- 이번 작업 범위는 **계획 확정과 구현 준비**를 포함한다.
- 실제 구현은 진행하되, 저장소 공개 배포와 운영 전환은 검증 완료 후 수행한다.
...
- 블로그 톤 결정
  - 미니멀한 시각 톤과 아카이브형 정보 구조의 조합으로 확정
- 공통 UI 요소 정의
...
- `diary`, `tweet`, `review` 섹션별 성격 유지 확인
- 첫 게시글은 블로그 소개로 준비
- URL 규칙은 slug 중심 방식으로 확정

### URL 규칙 권장안

- 실무 기준으로는 slug 중심 URL이 운영 부담이 가장 낮다.
- 이유는 주소가 짧고, 글 수정이나 재정리 시 URL 안정성을 유지하기 쉽기 때문이다.
- 날짜 정보는 주소에 넣기보다 글 메타데이터와 목록 UI에서 노출하는 편이 유연하다.
- 따라서 현재 플랜의 1순위 권장안은 slug 중심 URL이다.

## 6. 배포 전 보류 항목

- GitHub 저장소 실제 생성
- GitHub Actions 워크플로 작성 및 배포 설정 연동
- GitHub Pages 설정 적용
- 커스텀 도메인 연결
- 운영용 콘텐츠 추가 작성 및 배포
...
- Next.js 블로그 초기 스캐폴딩과 콘텐츠 구조 구현을 진행한다.
- 빌드/테스트 통과 후 GitHub Pages 배포 설정을 정리한다.
